...
def buscarproductos(request):
    if request.is_ajax():
        ...
        if texto is not None:
            ...
            lista = []
            for c in clase:
				lista.append({'pk':c.pk,'categoria':c.categoria.nombre, 'marca':c.modelo.marca.nombre ,'modelo':c.modelo.nombre,'barra':c.barra, 'costo':str(c.costo) ,'mayor':str(c.precio.mayor), 'punto':str(c.precio.punto), 'cliente':str(c.precio.cliente), 'stock':c.stock})
            data = json.dumps(lista)
            return HttpResponse(data, content_type='application/json')
